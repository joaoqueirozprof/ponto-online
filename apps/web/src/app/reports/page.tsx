...
const getMonthName = (month: number): string => MONTHS[month - 1]?.label || '';
...
const getBranchName = (branch: { name: string } | string): string =>
  typeof branch === 'string' ? branch : branch.name;

const getCompanyName = (branch: { company?: { name: string } } | string): string =>
  typeof branch === 'string' ? '' : branch.company?.name || '';
...
export default function ReportsPage() {
  ...
  const FormSelect = ({ label, value, onChange, children, required = true }: {
  ...
  // Get punch times string for a day
              ...
              {employeeLoading ? <LoadingSpinner /> : employeeData ? (
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
                    ...
                    <MetricCard icon="⚠️" label="Atraso" value={formatHHMM(employeeData.timesheet.totalLateMinutes)} variant={employeeData.timesheet.totalLateMinutes > 0 ? 'warning' : 'default'} />
                    <MetricCard icon="❌" label="Faltas" value={formatHHMM(employeeData.timesheet.totalAbsenceMinutes)} variant={employeeData.timesheet.totalAbsenceMinutes > 0 ? 'danger' : 'default'} />
                    <MetricCard icon="🏦" label="Banco Horas" value={formatHHMM(employeeData.timesheet.totalBalanceMinutes)} variant={employeeData.timesheet.totalBalanceMinutes >= 0 ? 'success' : 'danger'} />
                        ...
                        <tbody>
                          {employeeData.timesheet.timesheetDays.map((day, idx) => (
                            <tr key={idx} className={`${getDayRowClass(day.status, idx)} border-b border-slate-700/20 hover:bg-slate-600/20 transition print:border-gray-200 print:hover:bg-transparent`}>
                              ...
                              <td className="px-3 py-2 text-right text-emerald-400 font-mono print:text-green-700">{day.overtimeMinutes > 0 ? formatHHMM(day.overtimeMinutes) : '-'}</td>
                              <td className="px-3 py-2 text-right text-purple-400 font-mono print:text-purple-700">{day.nightMinutes > 0 ? formatHHMM(day.nightMinutes) : '-'}</td>
                              <td className="px-3 py-2 text-right text-amber-400 font-mono print:text-amber-700">{day.lateMinutes > 0 ? formatHHMM(day.lateMinutes) : '-'}</td>
                              <td className="px-3 py-2 text-right text-red-400 font-mono print:text-red-700">{day.absenceMinutes > 0 ? formatHHMM(day.absenceMinutes) : '-'}</td>
              ...
              {branchLoading ? <LoadingSpinner /> : branchData ? (
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <MetricCard icon="👥" label="Colaboradores" value={branchData.summary.totalEmployees} variant="info" />
                    <MetricCard icon="📊" label="Processados" value={branchData.summary.processedTimesheets} variant="default" />
                    <MetricCard icon="✅" label="Aprovados" value={branchData.summary.approvedTimesheets} variant="success" />
                    <MetricCard icon="⏱️" label="Média Horas" value={`${branchData.summary.averageWorkedHours.toFixed(1)}h`} variant="info" />
                    <MetricCard icon="⏰" label="Total Extra" value={`${branchData.summary.totalOvertimeHours.toFixed(1)}h`} variant="success" />
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {[...branchData.timesheets]
                            ...
                            .map((item, idx) => (
                              <tr key={idx} className={`${idx % 2 === 0 ? 'bg-slate-800/50' : 'bg-slate-750/30'} border-b border-slate-700/20 hover:bg-slate-600/20 transition print:bg-white print:border-gray-200`}>
                                ...
                                <td className="px-3 py-2 text-right font-semibold font-mono print:text-black" style={{ color: item.totalBalanceMinutes >= 0 ? '#34d399' : '#f87171' }}>
                          ...
                          <tr className="bg-slate-700/80 border-t-2 border-indigo-500/30 font-bold print:bg-gray-100 print:border-gray-400">
                            ...
                            <td className="px-3 py-3 text-right text-white font-mono print:text-black">
                              {formatHHMM(branchData.timesheets.reduce((s, t) => s + t.totalWorkedMinutes, 0))}
                            </td>
                            <td className="px-3 py-3 text-right text-emerald-400 font-mono print:text-green-700">
                              {formatHHMM(branchData.timesheets.reduce((s, t) => s + t.totalOvertimeMinutes, 0))}
                            </td>
                            <td className="px-3 py-3 text-right text-purple-400 font-mono print:text-purple-700">
                              {formatHHMM(branchData.timesheets.reduce((s, t) => s + t.totalNightMinutes, 0))}
                            </td>
                            <td className="px-3 py-3 text-right text-amber-400 font-mono print:text-amber-700">
                              {formatHHMM(branchData.timesheets.reduce((s, t) => s + t.totalLateMinutes, 0))}
                            </td>
                            <td className="px-3 py-3 text-right text-red-400 font-mono print:text-red-700">
                              {formatHHMM(branchData.timesheets.reduce((s, t) => s + t.totalAbsenceMinutes, 0))}
                            </td>
                            <td className="px-3 py-3 text-right font-mono" style={{ color: branchData.timesheets.reduce((s, t) => s + t.totalBalanceMinutes, 0) >= 0 ? '#34d399' : '#f87171' }}>
                              {formatHHMM(branchData.timesheets.reduce((s, t) => s + t.totalBalanceMinutes, 0))}
                            </td>
                            <td></td>
                          </tr>
                        </tbody>
            ...
            <div>
              {payrollLoading ? <LoadingSpinner /> : payrollData ? (
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <MetricCard icon="👥" label="Processados" value={payrollData.totalProcessed} variant="info" />
                    <MetricCard icon="⏱️" label="Total Horas" value={formatHHMM(payrollData.payrollData.reduce((s, p) => s + p.workedMinutes, 0))} variant="info" />
                    <MetricCard icon="⏰" label="Total Extra" value={formatHHMM(payrollData.payrollData.reduce((s, p) => s + p.overtimeMinutes, 0))} variant="success" />
                    <MetricCard icon="🌙" label="Total Noturnas" value={formatHHMM(payrollData.payrollData.reduce((s, p) => s + p.nightMinutes, 0))} variant="purple" />
                  </div>
                        ...
                        <tbody>
                          {payrollData.payrollData.map((item, idx) => (
                            <tr key={idx} className={`${idx % 2 === 0 ? 'bg-slate-800/50' : 'bg-slate-750/30'} border-b border-slate-700/20 hover:bg-slate-600/20 transition print:bg-white print:border-gray-200`}>
                              ...
                              <td className="px-3 py-2 text-right font-semibold font-mono print:text-black" style={{ color: item.balanceMinutes >= 0 ? '#34d399' : '#f87171' }}>
                          ...
                          {payrollData.payrollData.length > 0 && (
                            <tr className="bg-slate-700/80 border-t-2 border-indigo-500/30 font-bold print:bg-gray-100 print:border-gray-400">
                              ...
                              <td className="px-3 py-3 text-right text-white font-mono print:text-black">
                                {formatHHMM(payrollData.payrollData.reduce((s, p) => s + p.workedMinutes, 0))}
                              </td>
                              <td className="px-3 py-3 text-right text-emerald-400 font-mono print:text-green-700">
                                {formatHHMM(payrollData.payrollData.reduce((s, p) => s + p.overtimeMinutes, 0))}
                              </td>
                              <td className="px-3 py-3 text-right text-purple-400 font-mono print:text-purple-700">
                                {formatHHMM(payrollData.payrollData.reduce((s, p) => s + p.nightMinutes, 0))}
                              </td>
                              <td className="px-3 py-3 text-right text-amber-400 font-mono print:text-amber-700">
                                {formatHHMM(payrollData.payrollData.reduce((s, p) => s + p.lateMinutes, 0))}
                              </td>
                              <td className="px-3 py-3 text-right text-red-400 font-mono print:text-red-700">
                                {formatHHMM(payrollData.payrollData.reduce((s, p) => s + p.absenceMinutes, 0))}
                              </td>
                              <td className="px-3 py-3 text-right font-mono" style={{ color: payrollData.payrollData.reduce((s, p) => s + p.balanceMinutes, 0) >= 0 ? '#34d399' : '#f87171' }}>
                                {formatHHMM(payrollData.payrollData.reduce((s, p) => s + p.balanceMinutes, 0))}
                              </td>
                            </tr>
                          )}
                        </tbody>
